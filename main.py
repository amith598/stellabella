...
async def Lycia(_, message):
    ...
    try:
        L = await fetch(f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Lycia&ownername=NOOBX7&user=1")
        chatbot = L["message"]
        VoiceAi = f"https://stellabella.herokuapp.com/lycia?text={chatbot}&lang=en"
        name = "lycia"
    except Exception as e:
        await m.edit(str(e))
        return
    await m.edit("Made By @NOOBX7...")
